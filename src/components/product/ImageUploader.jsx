    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {images.map((img) => (
          <div
            ...
            className="relative aspect-square rounded-xl overflow-hidden bg-walnut-brown/5 group border border-walnut-brown/10"
          >
            <img
              src={img.url}
              alt="product"
      ...
      {images.length === 0 && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-50 border border-amber-200/50 text-amber-800 text-xs font-medium">
          <HiPhotograph size={16} className="shrink-0" />
          <span>
            Upload at least 1 image. Drafts with no photos will show fallbacks.
          </span>
        </div>
      )}
    </div>
