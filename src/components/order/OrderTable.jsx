  ...
  if (orders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-white border border-walnut-brown/15 rounded-2xl">
        <span className="text-sm font-semibold text-charcoal-text/50 uppercase">No orders logged</span>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-walnut-brown/10">
          ...
          <tbody className="divide-y divide-walnut-brown/5 text-sm text-charcoal-text">
            {orders.map((ord) => {
              const formattedDate = new Date(ord.createdAt).toLocaleDateString('en-KE', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
              });
              ...
              return (
                <tr key={ord._id} className="hover:bg-walnut-brown/2 transition-colors">
                  <td className="px-6 py-4 font-bold text-walnut-brown">
                    #{ord.orderNumber || ord._id.substring(18)}
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-semibold text-charcoal-text">{ord.customerName}</div>
                    <div className="text-xs text-charcoal-text/50 font-medium">{ord.customerPhone}</div>
                  </td>
                  <td className="px-6 py-4 font-medium max-w-xs truncate">
                    {ord.productId?.name || 'Unknown Item'}
                  </td>
                  <td className="px-6 py-4 font-extrabold text-walnut-brown">
                    {formatCurrency(ord.negotiatedPrice)}
                  </td>
